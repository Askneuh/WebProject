...
async function fetchUsers() {
  const res = await fetch('https://boorinthe-back.cluster-ig3.igpolytech.fr/admin/users', { credentials: 'include', headers: { 'Content-Type': 'application/json' } });
  if (!res.ok) {
...
window.deleteUser = async function(id) {
  ...
  const res = await fetch(`https://boorinthe-back.cluster-ig3.igpolytech.fr/admin/users/${id}`, {
    method: 'DELETE',
    credentials: 'include'
  });
  if (res.ok) {
